...
int main(int argc, char **argv) {
    ...
    mem[0x0028] = 0x00; mem[0x0029] = 0x08; /* BASL/BASH: buffer pointer -- point
        at safe scratch memory ($0800+), NOT $0000 (zero page!). Textalker uses
        STA ($28),Y to buffer characters; leaving this at its zero default meant
        every buffered character was silently corrupting zero page (including
        Textalker's own internal state), which explains several of the odder
        failures we'd been chasing. */
    ...
    for (long i = 0; i < inlen; i++) {
        ...
        int s = run_to_halt(0xD006, 20000000L, 0x0201, 1, ch);
        total_steps += s;
